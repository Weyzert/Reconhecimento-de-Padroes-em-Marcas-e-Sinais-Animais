...
#Remoção de ruído   USAR O SOBEL SOBRE O BINARIZADA GERA UMA IMAGEM HUMANAMENTE MAIS AGRADAVEL que sobre o gaussinizada
        # Sobel
...
def main():
    # Obtem o diretório do script (diretório onde o script está localizado)
    script_directory = os.path.dirname(os.path.realpath(__file__))
    #print(script_directory)
    
    # Caminho para a pasta de imagens
    images_folder_path = os.path.join(script_directory, 'imagens')
    ...
    # Carrega a imagem de query
    nome_img_query = 'm.png'
    query_image_path = os.path.join(images_folder_path, nome_img_query)
    query_image = cv2.imread(query_image_path, cv2.IMREAD_GRAYSCALE)
    #exibir_img("query_image", query_image)

    # Compara as imagens e obter a lista ordenada pelo grau de semelhança
    matches_list = compare_images(query_image, image_list)
    print("-" * 32)
    print("Comparação feita em: ", nome_img_query)
    print("-" * 32)
    for match in matches_list:
        print(f"Nome: {match['nome']}")
        print(f"Índice: {match['extensao']}")
        print(f"Índice: {match['index']}")
        print(f'Caminho: {match["path"]}')
        print(f'Similaridade (Flann): {match["flann_similarity"]:.4f}')
        print(f'Similaridade Percentual (Flann): {match["flann_similarity_percent"]:.2f}%')
        print(f'Índice Euclidiano: {match["euclidean_and_cosine_index"]}')
        print(f'Distância Euclidiana: {match["euclidean_distance"]:.4f}') # o quanto mais proximo de 0 melhor é (e menor é a distancia)
        print(f'Similaridade Cosseno: {match["cosine_similarity"]:.4f}') # -1 = totalmente opostos, 1 idênticos, 0 ortogonais
        print(f'Similaridade Percentual Cityblock: {match["cityblock_similarity_percent"]:.2f}%') # Quanto menor for o valor retornado, maior será a similaridade. Se o valor for zero, significa que os dois vetores (ou imagens) são idênticos. Quanto maior o valor, menor é a similaridade entre os vetores.
        print(f'OOE: {match["original_order_euclidiana"]}')
        print(f'OOC: {match["original_order_cosine"]}')
        print("-" * 40)
